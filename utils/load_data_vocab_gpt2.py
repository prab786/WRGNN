...
def load_gpt2_tokenizer():
    """Load GPT-2 tokenizer with padding token"""
...
def tokenize_gpt2(texts, tokenizer):
...
def build_pyg_graph(texts, tokenizer=tokenizer, window_size=10, min_count=1):
    """Build PyG graph with GPT-2-compatible node features"""
    tokenized = tokenize_gpt2(texts, tokenizer)
    ...
    if not edges:
        ...
        # Create minimal graph with GPT-2-sized features
        x = torch.randn(max(1, len(token2idx)), 768)  # GPT-2 embedding size
    ...
    # Create GPT-2-compatible node features
    x = torch.randn(len(token2idx), 768)  # GPT-2 embedding dimension
...
def build_pyg_graph_with_gpt2_init(texts, tokenizer=tokenizer, window_size=10, min_count=1):
    """Build PyG graph and initialize with actual GPT-2 embeddings"""
    tokenized = tokenize_gpt2(texts, tokenizer)
    vocab, token2idx, counter, cooc, total_windows = build_vocab_and_cooc(tokenized, window_size, min_count)
    edges, weights = compute_npmi_edges(token2idx, counter, cooc, total_windows)
    
    print(f"Building graph with GPT-2 initialization: {len(token2idx)} tokens, {len(edges)} edges")
    
    if not edges:
        print("No edges found, creating minimal graph")
        x = torch.randn(max(1, len(token2idx)), 768)
        edge_index = torch.zeros(2, 0, dtype=torch.long)
        edge_weight = torch.zeros(0, dtype=torch.float)
        return Data(x=x, edge_index=edge_index, edge_attr=edge_weight), token2idx
    ...
    # Initialize node features with GPT-2 embeddings
    print("Initializing node features with GPT-2 embeddings...")
    gpt2_model = GPT2Model.from_pretrained('gpt2')
    gpt2_model.eval()
    
    # Create node features matrix
    x = torch.zeros(len(token2idx), 768)
    
    # Get GPT-2 embeddings for each token in vocabulary
    ...
    with torch.no_grad():
        for i, token in enumerate(tokens):
            ...
            outputs = gpt2_model(token_ids)
            # Use last token embedding as token representation (since GPT-2 doesn't have [CLS])
            token_embedding = outputs.last_hidden_state[0, -1, :]  # Last token
            
            vocab_idx = token2idx[token]
            x[vocab_idx] = token_embedding
    
    print(f"Initialized {len(token2idx)} node features with GPT-2 embeddings")
    return Data(x=x, edge_index=edge_index, edge_attr=edge_weight), token2idx

def build_pyg_graph_efficient(texts, tokenizer=tokenizer, window_size=10, min_count=1, 
                            node_feature_dim=768, use_gpt2_init=False):
    """
    Efficient graph building with proper node feature dimensions for GPT-2
    ...
    Args:
        ...
        node_feature_dim: Dimension of node features (default 768 for GPT-2)
        use_gpt2_init: Whether to initialize with GPT-2 embeddings (slower but better)
    """
    tokenized = tokenize_gpt2(texts, tokenizer)
    vocab, token2idx, counter, cooc, total_windows = build_vocab_and_cooc(tokenized, window_size, min_count)
    edges, weights = compute_npmi_edges(token2idx, counter, cooc, total_windows)
    ...
    if use_gpt2_init:
        print("Initializing with GPT-2 embeddings (this may take a while)...")
        x = initialize_with_gpt2_embeddings(token2idx, tokenizer, node_feature_dim)
    else:
...
def initialize_with_gpt2_embeddings(token2idx, tokenizer, embedding_dim=768):
    """Initialize node features with GPT-2 embeddings"""
    gpt2_model = GPT2Model.from_pretrained('gpt2')
    gpt2_model.eval()
        ...
        for i in range(0, len(tokens), batch_size):
            ...
            for token in batch_tokens:
                try:
                    ...
                    # Use last token embedding (GPT-2 style)
                    token_embedding = outputs.last_hidden_state[0, -1, :]
                    
                    vocab_idx = token2idx[token]
                    x[vocab_idx] = token_embedding[:embedding_dim]  # Ensure correct size
                    
                except Exception as e:
                    print(f"Warning: Could not get GPT-2 embedding for token '{token}': {e}")
...
def get_vocab_subgraph(token_ids, vocab_graph: Data, token2idx: dict, tokenizer) -> Data:
    ...
    Args:
        token_ids (List[int]): GPT-2 token IDs (from tokenizer).
...
def get_vocab_subgraph_from_text(text: str, vocab_graph: Data, token2idx: dict, tokenizer) -> Data:
    """
    Returns a subgraph of the vocab graph containing only GPT-2 tokens from the input text.
...
def get_vocab_subgraph_from_text_robust(text, vocab_graph, token2idx, tokenizer):
    """
    Robust version of get_vocab_subgraph_from_text with better error handling for GPT-2.
    """
    try:
...
def visualize_vocab_subgraph(text, vocab_graph, token2idx, tokenizer=tokenizer, top_n=20):
    ...
    if subgraph.edge_attr is not None:
        ...
        nx.draw_networkx_nodes(G_filtered, pos, node_color='lightgreen', node_size=1000, alpha=0.8)
        ...
        nx.draw_networkx_nodes(G, pos, node_color='lightgreen', node_size=1000, alpha=0.8)
    ...
    plt.title(f"GPT-2 Vocabulary Subgraph (Top {min(top_n, len(G.edges()))} Edges by Weight)", fontsize=16)
    plt.axis("off")
...
def map_spacy_to_gpt2(text, tokenizer, spacy_nlp):
    """
    Maps spaCy tokens to GPT-2 subword indices using character-based alignment.
    ...
    Returns:
        list: List of tuples (spacy_token_index, [gpt2_subword_indices]).
    """
    # Process text with spaCy
    doc = spacy_nlp(text)
    spacy_tokens = [token.text for token in doc]

    # Tokenize with GPT-2 and get character offsets
    encoded = tokenizer(text, return_offsets_mapping=True, add_special_tokens=False)
    gpt2_tokens = encoded.tokens()
    offsets = encoded.offset_mapping  # Each subword's (start, end) character positions

    # Map each character position to its corresponding GPT-2 token index
    char_to_gpt2 = {}
    for gpt2_idx, (start, end) in enumerate(offsets):
        for char_idx in range(start, end):  
            char_to_gpt2[char_idx] = gpt2_idx

    # Map spaCy tokens to corresponding GPT-2 subwords
    mapping_spacy_to_gpt2 = []
    for spacy_idx, token in enumerate(doc):
        token_start = token.idx  # Start position in text
        token_end = token_start + len(token)

        # Collect all GPT-2 subword indices that align with this token
        subword_indices = sorted(set(char_to_gpt2.get(i) for i in range(token_start, token_end) if i in char_to_gpt2))

        mapping_spacy_to_gpt2.append((spacy_idx, subword_indices))

    return mapping_spacy_to_gpt2

def construct_text_graph(text, save_path=None, tokenizer=tokenizer, spacy_nlp=nlp):
    """
    Constructs a graph representation of a text using GPT-2 tokens and spaCy dependencies.
    ...
    Returns:
        ...
        list: List of token IDs for GPT-2 tokens.
    ...
    # Process text with spaCy
    doc = spacy_nlp(text)
    spacy_dependencies = [(token.i, token.dep_, token.head.i) for token in doc]

    # Tokenize with GPT-2
    gpt2_tokens = tokenizer.tokenize(text)
    gpt2_token_ids = tokenizer.convert_tokens_to_ids(gpt2_tokens)

    # Get mapping from spaCy tokens to GPT-2 subwords
    mapping_spacy_to_gpt2 = map_spacy_to_gpt2(text, tokenizer, spacy_nlp)

    # Create GPT-2-based dependency graph
    edges = []
    edge_weights = []
    dependency_relations = []
    
    for token_idx, dep, head_idx in spacy_dependencies:
        token_gpt2_indices = [gpt2_idx for _, gpt2_indices in mapping_spacy_to_gpt2 if _ == token_idx for gpt2_idx in gpt2_indices]
        head_gpt2_indices = [gpt2_idx for _, gpt2_indices in mapping_spacy_to_gpt2 if _ == head_idx for gpt2_idx in gpt2_indices]

        for t_gpt2_idx in token_gpt2_indices:
            for h_gpt2_idx in head_gpt2_indices:
                edges.append([t_gpt2_idx, h_gpt2_idx])
    ...
    return graph_data, gpt2_token_ids, df
...
def load_articles(obj):
    ...
    x_train_graphr = load_vocab_graphs('gr/vocab_gpt2_r_'+obj+'.Rograph', news_o)
    x_train_graphf = load_vocab_graphs('gr/vocab_gpt2_f_'+obj+'.Rograph', news_1)

    return x_train, x_test, x_test_res, y_train, y_test, x_train_graphr, x_train_graphf

def load_vocab_graphs(file_path, texts, force_rebuild=False):
    """Load or build vocabulary graphs with correct GPT-2-sized features"""
            ...
            if isinstance(vocab_data, dict) and 0 in vocab_data and 1 in vocab_data:
                ...
                if graph.x is not None:
                    ...
                    if node_feature_dim == vocab_size:
                        ...
                        print(f"⚠️  Non-GPT-2 feature dimension ({node_feature_dim})! Forcing rebuild...")
                        force_rebuild = True
                    else:
    ...
    if force_rebuild or not file_exists(file_path):
        ...
        graph, token2idx = build_pyg_graph_efficient(
            ...
            window_size=10, 
            min_count=1,
            node_feature_dim=768,  # GPT-2 dimension
            use_gpt2_init=False    # Set to True for better initialization (slower)
        )
...
def load_reframing(obj):
    ...
    x_train_res1_graph = load_graphs('gr/gpt2_x_train_res1'+obj+'.Rograph', x_train_res1)
    x_train_res1_2_graph = load_graphs('gr/gpt2_x_train_res1_2'+obj+'.Rograph', x_train_res1_2)
    x_train_res2_graph = load_graphs('gr/gpt2_x_train_res2'+obj+'.Rograph', x_train_res2)
    x_train_res2_2_graph = load_graphs('gr/gpt2_x_train_res2_2'+obj+'.Rograph', x_train_res2_2)
