...
def align_spacy_to_roberta(text, tokenizer=tokenizer, spacy_nlp=nlp, debug=False):
    ...
    for roberta_idx, (rob_start, rob_end) in enumerate(offset_mapping):
        # Skip special tokens and empty spans
        if roberta_idx in special_token_positions or rob_start == rob_end:
            continue
        ...
        for spacy_idx, (spacy_start, spacy_end) in enumerate(spacy_spans):
            ...
            overlap_start = max(rob_start, spacy_start)
            overlap_end = min(rob_end, spacy_end)
    ...
    for spacy_idx, roberta_indices in enumerate(mapping_spacy_to_roberta):
        if not roberta_indices and spacy_idx < len(spacy_tokens):
            ...
            for rob_idx, (rob_start, rob_end) in enumerate(offset_mapping):
                if rob_idx in special_token_positions or rob_start == rob_end:
                    continue
                    
                rob_mid = (rob_start + rob_end) / 2
                distance = abs(rob_mid - spacy_mid)
                
                if distance < min_distance:
                    min_distance = distance
                    closest_roberta_idx = rob_idx
...
def load_articles(obj):
    ...
    x_train_graph = load_graphs('graph/newstrain_t_all_'+obj+'.Rograph', x_train)
    x_test_graph = load_graphs('graph/newstest_t_all_'+obj+'.Rograph', x_test)
    x_test_res_graph = load_graphs('graph/newstest_t_all_res_a_'+obj+'.Rograph', x_test_res)
...
def load_articles_all(obj):
    ...
    x_test_res_graphb = load_graphs('graph/newstest_t_all_res_b_'+obj+'.Rograph', x_test_resB)
    x_test_res_graphc = load_graphs('graph/newstest_t_all_res_c_'+obj+'.Rograph', x_test_resC)
    x_test_res_graphd = load_graphs('graph/newstest_t_all_res_d_'+obj+'.Rograph', x_test_resD)
