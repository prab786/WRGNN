...
def clean_edge_index1(edge_index,edge_type ,edge_batch=0, num_nodes=511):
    ...
    valid_mask = (
        ...
        (edge_index[1] >= min_node_indices) & (edge_index[1] < max_node_indices)   & (edge_type != 1 )   )
...
stanza_nlp = stanza.Pipeline(lang='en', processors='tokenize,pos,depparse')

def map_stanza_to_bert(text, tokenizer, stanza_nlp):
    """
    Maps Stanza tokens to BERT subword indices using character-based alignment.
    """
    doc = stanza_nlp(text)
    ...
    mapping_stanza_to_bert = []
    char_index = 0
    for stanza_idx, token in enumerate(stanza_tokens):
        ...
        mapping_stanza_to_bert.append((stanza_idx, subword_indices))
    
    return mapping_stanza_to_bert

def construct_text_graph(text, save_path=None, tokenizer=None, stanza_nlp=None):
    """
    Constructs a graph representation of a text using BERT tokens and Stanza dependencies.
    """
    global graph_counter

    doc = stanza_nlp(text)
    stanza_dependencies = [(word.id - 1, word.deprel, word.head - 1) for sentence in doc.sentences for word in sentence.words if word.head > 0]
    
    bert_tokens = tokenizer.tokenize(text)
    bert_token_ids = tokenizer.convert_tokens_to_ids(bert_tokens)
    
    mapping_stanza_to_bert = map_stanza_to_bert(text, tokenizer, stanza_nlp)
    
    edges = []
    edge_weights = []
    dependency_relations = []
    
    for token_idx, dep, head_idx in stanza_dependencies:
        token_bert_indices = [bert_idx for _, bert_indices in mapping_stanza_to_bert if _ == token_idx for bert_idx in bert_indices]
        head_bert_indices = [bert_idx for _, bert_indices in mapping_stanza_to_bert if _ == head_idx for bert_idx in bert_indices]
