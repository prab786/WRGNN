...
def construct_text_graph(text, tokenizer=tokenizer, spacy_nlp=nlp):
    ...
    # Tokenize with DeBERTa
    deberta_tokens = tokenizer.tokenize(text)
    deberta_token_ids = tokenizer.convert_tokens_to_ids(deberta_tokens)
    
    # Map spaCy tokens to DeBERTa subword indices
    mapping_spacy_to_deberta = []
    current_deberta_index = 0
    buffer = ''
    
    for spacy_idx, token in enumerate(spacy_tokens):
        subword_indices = []        
        while current_deberta_index < len(deberta_tokens):
            deberta_token = deberta_tokens[current_deberta_index]
            
            # For DeBERTa, no special prefix stripping needed
            buffer += deberta_token
            subword_indices.append(current_deberta_index)
            current_deberta_index += 1
            
            if buffer == token:
                buffer = ''  # Reset for next token
                break
        
        mapping_spacy_to_deberta.append((spacy_idx, subword_indices))

    # Create DeBERTa-based dependency graph
    dependency_graph_deberta = []
    edges = []
    
    for (token, dep, head, token_idx, head_idx) in spacy_dependencies:
        token_deberta_indices = [deberta_idx for spacy_idx, deberta_indices in mapping_spacy_to_deberta if spacy_idx == token_idx for deberta_idx in deberta_indices]
        head_deberta_indices = [deberta_idx for spacy_idx, deberta_indices in mapping_spacy_to_deberta if spacy_idx == head_idx for deberta_idx in deberta_indices]

        for t_deberta_idx in token_deberta_indices:
            for h_deberta_idx in head_deberta_indices:
                dependency_graph_deberta.append({
                    "token": token,
                    "dep": dep,
                    "head": head,
                    "token_deberta_id": deberta_token_ids[t_deberta_idx],
                    "head_deberta_id": deberta_token_ids[h_deberta_idx]
                })
                edges.append([t_deberta_idx, h_deberta_idx])

    # Construct adjacency matrix for the graph
    ed = []
    ed_weights = []
    for i, token_id1 in enumerate(deberta_token_ids):
        for j, token_id2 in enumerate(deberta_token_ids):
    ...
    return graph_data, deberta_token_ids
...
def load_articles(obj):
    ...
    restyle_dict = pickle.load(open('data/adversarial_test/' + obj+ '_test_adv_B.pkl', 'rb'))
    ...
    x_test_res_graph=load_graphs('test_res__b'+obj+'.Rograph',x_test_res)
