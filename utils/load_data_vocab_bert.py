...
def build_pyg_graph(texts, tokenizer=tokenizer, window_size=10, min_count=1):
    """Build PyG graph with BERT-compatible node features"""
    ...
    if not edges:
        ...
        # Create minimal graph with BERT-sized features
        x = torch.randn(max(1, len(token2idx)), 768)  # BERT embedding size
    ...
    # CRITICAL FIX: Create BERT-compatible node features instead of massive identity matrix
    # Option 1: Random initialization (will be replaced by BERT embeddings anyway)
    x = torch.randn(len(token2idx), 768)  # BERT embedding dimension
    
    # Option 2: Alternative - use smaller learnable embeddings
    # x = torch.randn(len(token2idx), 256)  # Smaller dimension if you prefer
    
    # Option 3: Zero initialization
...
def build_pyg_graph_with_bert_init(texts, tokenizer=tokenizer, window_size=10, min_count=1):
    """Build PyG graph and initialize with actual BERT embeddings"""
    ...
    print(f"Building graph with BERT initialization: {len(token2idx)} tokens, {len(edges)} edges")
    ...
    # Initialize node features with BERT embeddings
    print("Initializing node features with BERT embeddings...")
    ...
    # Get BERT embeddings for each token in vocabulary
    ...
    with torch.no_grad():
        for i, token in enumerate(tokens):
            ...
            token_embedding = outputs.last_hidden_state[0, 0, :]  # [CLS] token
            
            vocab_idx = token2idx[token]
            x[vocab_idx] = token_embedding
    
    print(f"Initialized {len(token2idx)} node features with BERT embeddings")
    return Data(x=x, edge_index=edge_index, edge_attr=edge_weight), token2idx


def build_pyg_graph_efficient(texts, tokenizer=tokenizer, window_size=10, min_count=1, 
                            node_feature_dim=768, use_bert_init=False):
    """
    ...
    Args:
        ...
        node_feature_dim: Dimension of node features (default 768 for BERT)
        use_bert_init: Whether to initialize with BERT embeddings (slower but better)
    """
    tokenized = tokenize_bert(texts, tokenizer)
    vocab, token2idx, counter, cooc, total_windows = build_vocab_and_cooc(tokenized, window_size, min_count)
    edges, weights = compute_npmi_edges(token2idx, counter, cooc, total_windows)
    ...
    if use_bert_init:
        print("Initializing with BERT embeddings (this may take a while)...")
        x = initialize_with_bert_embeddings(token2idx, tokenizer, node_feature_dim)
    else:
...
def initialize_with_bert_embeddings(token2idx, tokenizer, embedding_dim=768):
    """Initialize node features with BERT embeddings"""
    from transformers import BertModel
        ...
        for i in range(0, len(tokens), batch_size):
            ...
            for token in batch_tokens:
                try:
                    ...
                    # Use average of all token embeddings (or just [CLS])
                    ...
                    print(f"Warning: Could not get BERT embedding for token '{token}': {e}")
...
def load_articles(obj):
    ...
    # fout.close()
    x_trian_graphr=load_vocab_graphs('gr/vocab_bert_r_'+obj+'.Rograph',news_o)
    x_trian_graphf=load_vocab_graphs('gr/vocab_bert_f_'+obj+'.Rograph',news_1)
...
def load_vocab_graphs(file_path, texts, force_rebuild=False):
    """Load or build vocabulary graphs with correct BERT-sized features"""
            ...
            if isinstance(vocab_data, dict) and 0 in vocab_data and 1 in vocab_data:
                ...
                if graph.x is not None:
                    ...
                    if node_feature_dim == vocab_size:
                        ...
                        print(f"⚠️  Non-BERT feature dimension ({node_feature_dim})! Forcing rebuild...")
                        force_rebuild = True
                    else:
    ...
    if force_rebuild or not file_exists(file_path):
        ...
        graph, token2idx = build_pyg_graph_efficient(
            ...
            window_size=10, 
            min_count=1,
            node_feature_dim=768,  # BERT dimension
            use_bert_init=False    # Set to True for better initialization (slower)
        )
...
# print(x_test[0][:250])
# visualize_vocab_subgraph(x_test[0][:250], x_trian_graphr[0], x_trian_graphr[1], tokenizer)
