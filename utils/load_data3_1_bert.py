...
def truncate_text_for_bert(text, tokenizer=tokenizer, max_length=MAX_SEQUENCE_LENGTH):
    """
    Truncate text to fit within BERT's maximum sequence length.
    Accounts for special tokens (CLS, SEP).
    """
    ...
    max_text_tokens = max_length - 2  # Reserve space for CLS and SEP tokens
...
def align_spacy_to_bert(text, tokenizer=tokenizer, spacy_nlp=nlp, debug=False):
    """
    Aligns spaCy tokens with BERT subwords using offset mapping.
    This function maps each spaCy token to the corresponding BERT subword token(s).
    ...
    Returns:
        mapping_spacy_to_bert: List where index i contains BERT token indices for spaCy token i
        text: The (potentially truncated) text that was processed
    """
    # Truncate text first to ensure it fits in BERT
    original_text = text
    text = truncate_text_for_bert(text, tokenizer)
    
    # Process text with spaCy
    doc = spacy_nlp(text)
    spacy_tokens = [token.text for token in doc]

    # Tokenize with BERT and get offset mappings
    ...
    # Initialize mapping: each spaCy token maps to a list of BERT token indices
    mapping_spacy_to_bert = [[] for _ in spacy_tokens]
    ...
    if debug:
        ...
        print(f"BERT tokens: {[(i, tok, offset_mapping[i]) for i, tok in enumerate(bert_tokens) if i not in special_token_positions]}")
    ...
    for bert_idx, (bert_start, bert_end) in enumerate(offset_mapping):
        ...
        if bert_idx in special_token_positions or bert_start == bert_end:
            continue
            
        # Find which spaCy tokens overlap with this BERT token
        best_overlap = 0
        best_spacy_idx = -1
        
        for spacy_idx, (spacy_start, spacy_end) in enumerate(spacy_spans):
            # Check for character span overlap
            overlap_start = max(bert_start, spacy_start)
            overlap_end = min(bert_end, spacy_end)
        ...
        # Assign BERT token to the best matching spaCy token
        if best_spacy_idx >= 0:
            mapping_spacy_to_bert[best_spacy_idx].append(bert_idx)
    
    # Handle unmapped spaCy tokens by finding closest BERT tokens
    for spacy_idx, bert_indices in enumerate(mapping_spacy_to_bert):
        if not bert_indices and spacy_idx < len(spacy_tokens):
            if debug:
                print(f"Warning: spaCy token '{spacy_tokens[spacy_idx]}' at {spacy_spans[spacy_idx]} has no BERT mapping")
            
            # Try to find the closest BERT token
            ...
            for bert_idx, (bert_start, bert_end) in enumerate(offset_mapping):
                ...
                bert_mid = (bert_start + bert_end) / 2
                distance = abs(bert_mid - spacy_mid)
                
                if distance < min_distance:
                    min_distance = distance
                    closest_bert_idx = bert_idx
            
            if closest_bert_idx >= 0:
                mapping_spacy_to_bert[spacy_idx].append(closest_bert_idx)
                if debug:
                    print(f"  -> Mapped to closest BERT token {closest_bert_idx}")
    
    if debug:
        print("Final mapping:")
        for i in range(len(spacy_tokens)):
            print(f"  spaCy[{i}] '{spacy_tokens[i]}' -> BERT{mapping_spacy_to_bert[i]}")
    
    return mapping_spacy_to_bert, text

def construct_text_graph(text, tokenizer=tokenizer, spacy_nlp=nlp):
    """
    Construct text graph with proper truncation handling.
    """
    # Truncate text first
    text = truncate_text_for_bert(text, tokenizer)
    ...
    # Tokenize with BERT with proper truncation
    bert_encoding = tokenizer(text, 
                            return_offsets_mapping=True,
                            max_length=MAX_SEQUENCE_LENGTH,
                            truncation=True,
                            add_special_tokens=True)
    ...
    bert_token_ids = bert_encoding.input_ids
    
    # Map spaCy tokens to BERT subword indices
    mapping_spacy_to_bert, processed_text = align_spacy_to_bert(text, tokenizer, spacy_nlp)
    
    # Create BERT-based dependency graph
    edges = []

    for (token, dep, head, token_idx, head_idx) in spacy_dependencies:
        if token_idx >= len(mapping_spacy_to_bert) or head_idx >= len(mapping_spacy_to_bert):
            continue  # Skip if indices are out of bounds
            
        token_bert_indices = mapping_spacy_to_bert[token_idx]
        head_bert_indices = mapping_spacy_to_bert[head_idx]

        for t_bert_idx in token_bert_indices:
            for h_bert_idx in head_bert_indices:
                # Ensure indices are within bounds
                if t_bert_idx < len(bert_token_ids) and h_bert_idx < len(bert_token_ids):
                    edges.append([t_bert_idx, h_bert_idx])
    
    # Construct adjacency matrix for the graph
    ed = []
    ed_weights = []
    actual_num_nodes = len(bert_token_ids)
...
def load_articles(obj):
    ...
    x_train_graph = load_graphs('graph/newstrain_t_all_'+obj+'.Bertgraph', x_train)
    x_test_graph = load_graphs('graph/newstest_t_all_'+obj+'.Bertgraph', x_test)
    x_test_res_graph = load_graphs('graph/newstest_t_all_res_a_'+obj+'.Bertgraph', x_test_res)
...
def load_articles_all(obj):
    ...
    x_test_res_graphb = load_graphs('graph/newstest_t_all_res_b_'+obj+'.Bertgraph', x_test_resB)
    x_test_res_graphc = load_graphs('graph/newstest_t_all_res_c_'+obj+'.Bertgraph', x_test_resC)
    x_test_res_graphd = load_graphs('graph/newstest_t_all_res_d_'+obj+'.Bertgraph', x_test_resD)
...
def load_graphs(file_path, texts):
    ...
    for i, text in enumerate(texts):
        ...
        try:
            # Truncate text to ensure it fits within BERT limits
...
def load_reframing(obj):
    ...
    x_train_res1_graph = load_graphs('graph/x_train_res1_all_'+obj+'.Bertgraph', x_train_res1)
    x_train_res1_2_graph = load_graphs('graph/x_train_res1_all_2_'+obj+'.Bertgraph', x_train_res1_2)
    x_train_res2_graph = load_graphs('graph/x_train_res2_all_'+obj+'.Bertgraph', x_train_res2)
    x_train_res2_2_graph = load_graphs('graph/x_train_res2_all_2_'+obj+'.Bertgraph', x_train_res2_2)
...
def test_alignment(text_sample="The quick brown fox jumps over the lazy dog.", debug=True):
    """
    Test the spaCy-BERT alignment with a sample text.
    """
    print("=" * 50)
    print("TESTING SPACY-BERT ALIGNMENT")
    print("=" * 50)
    
    mapping, processed_text = align_spacy_to_bert(text_sample, debug=debug)
    ...
    for i, token in enumerate(doc):
        bert_indices = mapping[i] if i < len(mapping) else []
        bert_subtokens = [bert_tokens[idx] for idx in bert_indices if idx < len(bert_tokens)]
        print(f"  {i}: '{token.text}' [{token.idx}:{token.idx + len(token.text)}] -> {bert_indices} {bert_subtokens}")
    
    print(f"\nBERT tokens ({len(bert_tokens)}):")
    for i, (token, offset) in enumerate(zip(bert_tokens, encoded["offset_mapping"])):
...
def validate_alignment_quality(text_samples):
    ...
    for i, text in enumerate(text_samples[:5]):  # Test first 5 samples
        ...
        mapping, _ = align_spacy_to_bert(text, debug=False)
...
def transform_loaded_graphs(file_path, texts):
    ...
    transformed_file_path = file_path.replace('.Bertgraph', '_edge2node.Bertgraph')
