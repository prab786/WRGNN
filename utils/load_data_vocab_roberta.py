...
def build_pyg_graph(texts, tokenizer=tokenizer, window_size=10, min_count=1):
    """Build PyG graph with RoBERTa-compatible node features"""
    ...
    if not edges:
        ...
        # Create minimal graph with RoBERTa-sized features
        x = torch.randn(max(1, len(token2idx)), 768)  # RoBERTa embedding size
    ...
    # CRITICAL FIX: Create RoBERTa-compatible node features instead of massive identity matrix
    # Option 1: Random initialization (will be replaced by RoBERTa embeddings anyway)
    x = torch.randn(len(token2idx), 768)  # RoBERTa embedding dimension
...
def build_pyg_graph_with_roberta_init(texts, tokenizer=tokenizer, window_size=10, min_count=1):
    """Build PyG graph and initialize with actual RoBERTa embeddings"""
    ...
    print(f"Building graph with RoBERTa initialization: {len(token2idx)} tokens, {len(edges)} edges")
    ...
    print("Initializing node features with RoBERTa embeddings...")
    ...
    # Get RoBERTa embeddings for each token in vocabulary
    ...
    with torch.no_grad():
        for i, token in enumerate(tokens):
            ...
            # Use <s> token embedding as token representation (RoBERTa's equivalent to [CLS])
            token_embedding = outputs.last_hidden_state[0, 0, :]  # <s> token
            
            vocab_idx = token2idx[token]
            x[vocab_idx] = token_embedding
    
    print(f"Initialized {len(token2idx)} node features with RoBERTa embeddings")
    return Data(x=x, edge_index=edge_index, edge_attr=edge_weight), token2idx
...
def build_pyg_graph_efficient(texts, tokenizer=tokenizer, window_size=10, min_count=1, 
    ...
    Args:
        ...
        node_feature_dim: Dimension of node features (default 768 for RoBERTa)
        use_roberta_init: Whether to initialize with RoBERTa embeddings (slower but better)
    """
    tokenized = tokenize_roberta(texts, tokenizer)
    vocab, token2idx, counter, cooc, total_windows = build_vocab_and_cooc(tokenized, window_size, min_count)
    edges, weights = compute_npmi_edges(token2idx, counter, cooc, total_windows)
        ...
        for i in range(0, len(tokens), batch_size):
            ...
            for token in batch_tokens:
                try:
                    ...
                    # Use average of all token embeddings (or just <s>)
...
def get_vocab_subgraph(token_ids, vocab_graph: Data, token2idx: dict, tokenizer) -> Data:
    ...
    Args:
        token_ids (List[int]): RoBERTa token IDs (from tokenizer).
...
def get_vocab_subgraph_from_text(text: str, vocab_graph: Data, token2idx: dict, tokenizer) -> Data:
    """
    Returns a subgraph of the vocab graph containing only RoBERTa tokens from the input text.
...
def map_spacy_to_roberta(text, tokenizer, spacy_nlp):
    """
    Maps spaCy tokens to RoBERTa subword indices using character-based alignment.
    ...
    Returns:
        list: List of tuples (spacy_token_index, [roberta_subword_indices]).
    """
    ...
    # Tokenize with RoBERTa and get character offsets
    encoded = tokenizer(text, return_offsets_mapping=True, add_special_tokens=False)
    roberta_tokens = encoded.tokens()
    offsets = encoded.offset_mapping  # Each subword's (start, end) character positions

    # Map each character position to its corresponding RoBERTa token index
    char_to_roberta = {}
    for roberta_idx, (start, end) in enumerate(offsets):
        for char_idx in range(start, end):  
            char_to_roberta[char_idx] = roberta_idx

    # Map spaCy tokens to corresponding RoBERTa subwords
    mapping_spacy_to_roberta = []
    for spacy_idx, token in enumerate(doc):
        token_start = token.idx  # Start position in text
        token_end = token_start + len(token)

        # Collect all RoBERTa subword indices that align with this token
        subword_indices = sorted(set(char_to_roberta.get(i) for i in range(token_start, token_end) if i in char_to_roberta))
...
def construct_text_graph(text, save_path=None,tokenizer=tokenizer, spacy_nlp=nlp):
    """
    Constructs a graph representation of a text using RoBERTa tokens and spaCy dependencies.
    ...
    Returns:
        ...
        list: List of token IDs for RoBERTa tokens.
    ...
    # Get mapping from spaCy tokens to RoBERTa subwords
    mapping_spacy_to_roberta = map_spacy_to_roberta(text, tokenizer, spacy_nlp)
    ...
    for token_idx, dep, head_idx in spacy_dependencies:
        token_roberta_indices = [roberta_idx for _, roberta_indices in mapping_spacy_to_roberta if _ == token_idx for roberta_idx in roberta_indices]
        head_roberta_indices = [roberta_idx for _, roberta_indices in mapping_spacy_to_roberta if _ == head_idx for roberta_idx in roberta_indices]
    ...
    return graph_data, roberta_token_ids, df
...
def load_articles(obj):
    ...
    x_trian_graphr=load_vocab_graphs('gr/vocab_roberta_r_'+obj+'.Rograph',news_o)
    x_trian_graphf=load_vocab_graphs('gr/vocab_roberta_f_'+obj+'.Rograph',news_1)
...
def load_vocab_graphs(file_path, texts, force_rebuild=False):
    ...
    if force_rebuild or not file_exists(file_path):
        ...
        graph, token2idx = build_pyg_graph_efficient(
            ...
            use_roberta_init=False    # Set to True for better initialization (slower)
        )
...
def build_pyg_graph_robust(texts, tokenizer=None, window_size=10, min_count=1):
    ...
    try:
        ...
        # Create node features (identity matrix as placeholder for RoBERTa embeddings)
...
def load_reframing(obj):
    ...
    x_train_res1_graph=load_graphs('gr/nnrobertatx_train_res1'+obj+'.Rograph',x_train_res1)
    x_train_res1_2_graph=load_graphs('gr/nnrobertatx_train_res1_2'+obj+'.Rograph',x_train_res1_2)
    x_train_res2_graph=load_graphs('gr/nnrobertatx_train_res2'+obj+'.Rograph',x_train_res2)
    x_train_res2_2_graph=load_graphs('gr/nnrobertatx_train_res2_2'+obj+'.Rograph',x_train_res2_2)
