...
from utils.load_data3_1_bert import *
...
parser.add_argument('--model_name', default='final-WR-BERTA-', type=str)
...
class InformationTheoreticLosses(nn.Module):
    """Add-on module for information-theoretic losses"""
    def __init__(self, feature_dim):
        super().__init__()
        self.feature_dim = feature_dim
        
    def mutual_information_loss(self, real_features, fake_features, labels):
        """Maximize MI between features and labels, minimize between real/fake"""
        # MINE estimator - can plug into your existing features
        def mine_estimate(x, y):
            joint = torch.cat([x, y], dim=-1)
            marginal_y = y[torch.randperm(y.size(0))]
            marginal = torch.cat([x, marginal_y], dim=-1)
            
            T = nn.Sequential(
                nn.Linear(joint.size(-1), 64),
                nn.ReLU(),
                nn.Linear(64, 1)
            ).to(x.device)
            
            return T(joint).mean() - torch.log(T(marginal).exp().mean() + 1e-8)
        
        # Maximize MI(features, labels)
        label_emb = F.one_hot(labels, 2).float()
        mi_real_label = mine_estimate(real_features, label_emb)
        mi_fake_label = mine_estimate(fake_features, label_emb)
        
        # Minimize MI(real, fake) for better separation
        mi_real_fake = mine_estimate(real_features, fake_features)
        
        return -mi_real_label - mi_fake_label + mi_real_fake
...
class EdgeEmbedding(nn.Module):
    """Enhanced edge embedding module that creates rich edge representations"""
    def __init__(self, in_channels, out_channels):
    ...
    def forward(self, node_embeddings, edge_index):
        """
        Create edge embeddings from node pairs
        Args:
            node_embeddings: [num_nodes, in_channels]
            edge_index: [2, num_edges]
        Returns:
...
class MultiHeadAttentionEdgeEmb1(nn.Module):
    def __init__(self, in_channels, out_channels, num_heads=4):
        ...
        self.final_proj = Linear(in_channels, out_channels)
...
class NodeEmdebding(MessagePassing):
    def __init__(self,in_channels,out_channels):
        super(NodeEmdebding, self).__init__(aggr='sum')
        self.mlp = Seq(
            Linear(in_channels, out_channels),  # Only one in_channels because Hadamard reduces dimensionality
            ReLU(),
            LayerNorm(out_channels),
    ...
    def forward(self, x, edge_index, edge_weight):
        # x: Node features [num_nodes, feature_dim] (say, 768)
        # edge_index: [2, num_edges]
        # edge_weight: [num_edges, 768] -> embedding per edge
        return self.propagate(edge_index, x=x, edge_weight=edge_weight)

    def message(self, x_j, edge_weight):
        # x_j: [num_edges, 768] â€" source node features per edge
        # edge_weight: [num_edges, 768] â€" edge features
        return  edge_weight  # Element-wise multiplication

    def update(self, aggr_out):
        # aggr_out: [num_nodes, 768]
        return self.mlp(aggr_out)
from fusion import HierarchicalFusion,EnhancedConcatFusion
class BertClassifier(nn.Module):
    def __init__(self, n_classes):
        ...
        # # Unfreeze encoder layers 0 to 3
        # for i in range(3):  # layers 0, 1, 2, 3
        ...
        #self.ee=DifferenceEdgeEmb(self.bert.config.hidden_size,self.bert.config.hidden_size)
        self.ee=AttentionTrackingEdgeEmbedding(self.bert.config.hidden_size,self.bert.config.hidden_size) 
        self.ee1=AttentionTrackingEdgeEmbedding(self.bert.config.hidden_size,self.bert.config.hidden_size) 
        self.node_model1=NodeEmdebding(self.bert.config.hidden_size,self.bert.config.hidden_size)  
        self.ee2=AttentionTrackingEdgeEmbedding(self.bert.config.hidden_size,self.bert.config.hidden_size) 
        self.node_model2=NodeEmdebding(self.bert.config.hidden_size,self.bert.config.hidden_size) 
        self.fusion=nn.Sequential(
            nn.Linear(self.bert.config.hidden_size*3, 2*self.bert.config.hidden_size),
            GeGLU(),           
           )    
        ...
        self.graph_pool3 = AttentionalAggregation(
                gate_nn=nn.Sequential(
                    nn.Linear(self.bert.config.hidden_size, 128),
                    nn.GELU(),
                    nn.Linear(128, 1,bias=False)
        ...
        self.graph_pool4 = AttentionalAggregation(
                gate_nn=nn.Sequential(  
                    nn.Linear(self.bert.config.hidden_size, 128),
                    nn.GELU(),
                    nn.Linear(128, 1,bias=False)
        ...
        #self.graph_pool = GlobalAttention(gate_nn=self.gate_nn)

        self.node_model=NodeEmbedding(self.bert.config.hidden_size,self.bert.config.hidden_size)
        self.attention_pool = GlobalAttention(self.gate_nn)
        self.norm = nn.LayerNorm(self.bert.config.hidden_size)
        self.norm2= nn.LayerNorm(self.bert.config.hidden_size)
        self.attention=MaskAttention(self.bert.config.hidden_size)
        self.fc_out = nn.Linear(self.bert.config.hidden_size, 2)
        self.fusion1 = HierarchicalFusion(self.bert.config.hidden_size)
        self.fusion2 = EnhancedConcatFusion(self.bert.config.hidden_size)
        #self.binary_transform = nn.Linear(self.bert.config.hidden_size*2, 2)
        self.fc3=nn.Sequential(  
                    nn.Linear(3*self.bert.config.hidden_size, self.bert.config.hidden_size),
                    nn.GELU(),
    ...
    def forward(self, input_ids, attention_mask,graph_data):
        ...
        #x1 = all_hidden_states[9]
        # x1 = self.norm2(x1)
        # x1= self.custom_pooler(x1)

        #X2=all_hidden_states[9]
        ...
        #block1
        #x_updated = self.node_model1(x, edge_index, EdgeEmbedding)      
        EdgeEmbedding3 = self.ee1(x, edge_index)
        #block2
        # x_updated2 = self.node_model2(x, edge_index, EdgeEmbedding2)      
        # EdgeEmbedding4 = self.ee2(x_updated2, edge_index)
        #pooling
        ...
        #graph_embeddings3 = self.graph_pool3(EdgeEmbedding4, edge_batch)
       
        #g1=self.fusion1(graph_embeddings, graph_embeddings1,graph_embeddings2)
        #g2= self.fusion2(graph_embeddings1, graph_embeddings3)
        #rep = torch.cat((graph_embeddings, graph_embeddings1,graph_embeddings2), dim=1)
        #graph_embeddings2 = self.graph_pool2(EdgeEmbedding3, edge_batch)
        # graph_embeddings = self.dropout(graph_embeddings)
        #graph_embeddings1= self.dropout(graph_embeddings1)
        rep1 = torch.stack((graph_embeddings, graph_embeddings1,graph_embeddings2), dim=1)
        rep, scores = self.attention(rep1)
        #rep=torch.cat((rep, pooled_outputs), dim=1)
        #rep1=graph_embeddings1+graph_embeddings
        #rep = self.fusion(rep)
        #rep = self.fusion(rep)
        rep = self.norm(rep)
        output = self.fc(rep)
...
def get_optimizer_groups(model, learning_rate=2e-5, weight_decay=0.01):
    """Create optimizer parameter groups with different learning rates"""
    no_decay = ["bias", "LayerNorm.weight", "layer_norm.weight"]
    
    optimizer_grouped_parameters = [
        # BERT parameters (lower learning rate)
        {
        ...
        # GNN and fusion parameters (higher learning rate)
        {
...
def train_model(tokenizer, max_len, n_epochs, batch_size, datasetname, iter):
    ...
    model = BertClassifier(n_classes = 4).to(device)
    train_losses = []
    train_accs = []
    optimizer = AdamW(model.parameters(), lr=1e-5)
    # optimizer_groups = get_optimizer_groups(model)
    # optimizer = AdamW(optimizer_groups) # type: ignore
    ...
    for epoch in range(n_epochs):
        ...
        for Batch_data in tqdm(train_loader):
            ...
            x_train_res1_graphs=Batch_data['x_train_res1_graph'].to(device)
            ...
            x_train_res2_graphs=Batch_data['x_train_res2_graph'].to(device)
            out_labels, out_labels_bi,og,ogbi = model(input_ids=input_ids, attention_mask=attention_mask,graph_data=graph)
            # out_labels_aug1, out_labels_bi_aug1 ,og_aug1,og_aug1_bi = model(input_ids=input_ids_aug1, attention_mask=attention_mask_aug1,graph_data=x_train_res1_graphs)
            # out_labels_aug2, out_labels_bi_aug2 ,og_aug2,og_aug2_bi= model(input_ids=input_ids_aug2, attention_mask=attention_mask_aug2,graph_data=x_train_res2_graphs)
            # fg_criterion = nn.BCELoss()
        
            # finegrain_loss = (fg_criterion(F.sigmoid(og),fg_labels) + fg_criterion(F.sigmoid(og_aug1),fg_labels_aug1) + \
            #                    fg_criterion(F.sigmoid(og_aug2),fg_labels_aug2)) / 3
           

            # out_probs = F.softmax(ogbi, dim = -1)           
            # aug_log_prob1 = F.log_softmax(og_aug1_bi, dim = -1)
            # aug_log_prob2 = F.log_softmax(og_aug2_bi, dim = -1)
            

            sup_criterion = nn.CrossEntropyLoss()
            sup_loss = sup_criterion(ogbi, targets)
            # #sup_loss1 = sup_criterion(out_labels_bi, targets)
            ...
            #loss =  sup_loss+sup_loss2+finegrain_loss1+cons_loss1   #sup_loss  + sup_loss # +cons_loss++finegrain_loss
            loss = sup_loss #finegrain_loss+cons_loss+
            optimizer.zero_grad()
            loss.backward()
            ...
            optimizer.step()
            scheduler.step()
            #val_out_aug=torch.max(out_labels_bi,val_out_augg)
