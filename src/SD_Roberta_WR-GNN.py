...
class RobertaClassifier(nn.Module):
    def __init__(self, n_classes):
        ...
        #self.gat1 = GATConv(self.roberta.config.hidden_size, self.roberta.config.hidden_size, heads=1, concat=True)
        #self.ee=DifferenceEdgeEmb(self.roberta.config.hidden_size,self.roberta.config.hidden_size)
        ...
        self.fusion=nn.Sequential(
            nn.Linear(self.roberta.config.hidden_size*3, 2*self.roberta.config.hidden_size),
            GeGLU(),           
           )    
        self.gate_nn = torch.nn.Sequential(
        torch.nn.Linear(self.roberta.config.hidden_size, 1),  # Input feature dimension 
...
def get_optimizer_groups(model, learning_rate=2e-5, weight_decay=0.01):
    ...
    optimizer_grouped_parameters = [
        # RoBERTa parameters (lower learning rate)
        {
...
def train_model(tokenizer, max_len, n_epochs, batch_size, datasetname, iter):
    ...
    for epoch in range(n_epochs):
        ...
        for Batch_data in tqdm(train_loader):
            ...
            loss =finegrain_loss+cons_loss+sup_loss # 
            optimizer.zero_grad()
            loss.backward()
